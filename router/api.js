...
router.post('/refresh', function (req) {
...
router.post('/usuarios', function(req, res) {
    ...
    if (add) {
        if (regex.test(req.body.email)) { // validate email
            ...
            } else res.redirect('/crearUsuario?Err=2');
        } else res.redirect('/crearUsuario?Err=4');
    }
});
    ...
    models.Sesion.create({
        ...
        models.Sesion.findOne({
            ...
            models.Participants.create({
                ...
                res.redirect('/crearEscenario?idSesion=' +
                    results.id.toString() + '&created=0');
            });
        });
    });
});
...
router.post('/addVotes', function(req) {
...
router.get('/searchChat', function(req, res) {
    models.Chat.findAll({where: {sessionid:req.query.id}})
        .then(function(data) {
            res.send(data);
        });
});
...
router.get('/deleteSesion', function(req, res) {
    models.Stage.destroy({where: {SesionId: req.query.SessionId}})
        .then(function() {
            models.Sesion.destroy({where: {id: req.query.SessionId}})
...
router.get('/getTime',function(req,res) {
   models.Sesion.findOne({where: {id: req.query.id}})
       .then(function(ret) {res.send(ret)});
});
    ...
    models.Voto.findAll({
        ...
        .then(function(value) {
            ...
            for (var i = 0; i < value.length; i++) {
                if (value[i].priority === 0) a++;
                else if (value[i].priority === 1) b++;
                else if (value[i].priority === 2) c++;
                else if (value[i].priority === 3) d++;
            }
            res.send([a, b, c, d]);
        });
});
...
router.get('/Usuarios', function(req, res) {
  models.Participants.findAll({where: {SesionId: req.query.idSesion}})
      .then(function(response) {res.send(response)})
});

router.get('/getNameUser', function(req, res) {
   models.Usuario.findOne({where: {id: req.query.idUser}})
       .then(function(ret) {res.send(ret)})
});
